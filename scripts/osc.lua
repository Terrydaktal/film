...
local user_opts = {
    ...
    torrent_progress_file = "", -- verified torrent progress JSON to draw inside the seekbar
...
local function torrent_progress_file_for(path)
    if user_opts.torrent_progress_file ~= "" then
        return user_opts.torrent_progress_file
    end

    local torrent_dir = path and path:match("^(.-/stream_cache/[^/]+)")
    if torrent_dir then
        return torrent_dir .. "/.torrent_progress.json"
    end

    return path and (dirname(path) .. "/.torrent_progress.json") or nil
end

local function parse_torrent_ranges(content, key)
    local ranges = {}
...
local function torrent_progress_ranges()
    ...
    elseif contiguous_prefix and contiguous_prefix > 0 then
        ...
        fallback_ratio = limit_range(0, 1, downloaded / total)
    end

    if fallback_ratio and fallback_ratio > 0 then
        return {{start = 0, ["end"] = fallback_ratio * 100}}, total
    end
